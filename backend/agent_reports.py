...
def _evidence_markdown(snap):
    ...
    lines = [
        "## Attestation",
        f"Generated by Obserra — Cyber Crisis Commander for {snap.get('org_name') or 'the organization'} "
...
def _digest_trend_page(monthly):
    ...
    try:
        ...
        c.drawString(0.9 * inch, 0.7 * inch,
                     "Obserra \u2014 Cyber Crisis Commander \u00b7 figures computed from the live Defensibility Ledger.")
        c.showPage(); c.save()
        return buf.getvalue()
    except Exception:
        return b""
...
def _control_assurance_chart_page(monthly):
    ...
    try:
        ...
        c.setFillColor(colors.grey); c.setFont("Helvetica", 7.5)
        c.drawString(0.9 * inch, 0.7 * inch,
                     "Obserra \u2014 Cyber Crisis Commander \u00b7 computed from the live fire-drill ledger.")
        c.showPage(); c.save()
        return buf.getvalue()
    except Exception:
        return b""
...
async def _build_board_digest(org):
    ...
    # Go-Live production-readiness — live score + 7-day trend (real daily history)
    gl_score = gl_ready = None
    gl_trend = []
    try:
        from sap_uac import compute_go_live
        gl = await compute_go_live(oid)
        gl_score, gl_ready = gl.get("score"), gl.get("ready")
        gl_trend = [t["score"] for t in (gl.get("trend") or [])][-7:]
    except Exception:
        pass

    def _spark(vals):
        blocks = "\u2581\u2582\u2583\u2584\u2585\u2586\u2587\u2588"
        return "".join(blocks[min(7, max(0, int(v / 100 * 7)))] for v in vals)

    gl_col = "#16a34a" if gl_ready else ("#d97706" if gl_score is not None else "#6b7280")
    gl_val = (f"{gl_score}%{' · ready' if gl_ready else ''}" if gl_score is not None else "\u2014")
    trend_html = ""
    if len(gl_trend) >= 2:
        trend_html = (f"<div style='font:600 12px Arial;color:#374151;margin-top:10px'>Production-readiness \u2014 last {len(gl_trend)} day(s): "
    ...
    html = (f"<div style='font:400 14px Arial;color:#1f2937;max-width:600px;margin:auto'>"
            f"<h2 style='color:#0f1e3d;margin-bottom:2px'>Cyber Crisis Commander — Board Evidence Digest</h2>"
            ...
            f"<p style='font-size:11px;color:#9ca3af'>Obserra — Cyber Crisis Commander · Defensibility Ledger</p></div>")
    ...
    counts = {"kills": kills, "suspends": suspends, "resumes": resumes, "toxic": c.get("toxic", 0),
              ...
              "questions_open": open_qs, "questions_total": len(qa), "go_live_score": gl_score}
    return {"html": html, "subject": f"Cyber Crisis Commander — Board Evidence Digest ({now.strftime('%b %Y')})",
    ...
    for room in rooms:
        try:
            ...
            html = (f"<div style='font:400 14px Arial;color:#1f2937;max-width:560px;margin:auto'>"
                    ...
                    f"<p style='font-size:11px;color:#9ca3af'>You subscribed to weekly updates for this read-only auditor room. Obserra — Cyber Crisis Commander.</p></div>")
            for em in subs:
                try:

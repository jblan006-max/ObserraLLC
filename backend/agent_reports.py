...
def _evidence_markdown(snap):
    ...
    lines = [
        "## Attestation",
        f"Generated by Obserra — Control Intelligence for {snap.get('org_name') or 'the organization'} "
...
def _digest_trend_page(monthly):
    ...
    try:
        ...
        c.drawString(0.9 * inch, 0.7 * inch,
                     "Obserra \u2014 Control Intelligence \u00b7 figures computed from the live Defensibility Ledger.")
        c.showPage(); c.save()
        return buf.getvalue()
    except Exception:
        return b""
...
def _control_assurance_chart_page(monthly):
    ...
    try:
        ...
        c.setFillColor(colors.grey); c.setFont("Helvetica", 7.5)
        c.drawString(0.9 * inch, 0.7 * inch,
                     "Obserra \u2014 Control Intelligence \u00b7 computed from the live fire-drill ledger.")
        c.showPage(); c.save()
        return buf.getvalue()
    except Exception:
        return b""
...
async def _build_board_digest(org):
    ...
    html = (f"<div style='font:400 14px Arial;color:#1f2937;max-width:600px;margin:auto'>"
            f"<h2 style='color:#0f1e3d;margin-bottom:2px'>Control Intelligence — Board Evidence Digest</h2>"
            ...
            f"<p style='font-size:11px;color:#9ca3af'>Obserra — Control Intelligence · Defensibility Ledger</p></div>")
    ...
    return {"html": html, "subject": f"Control Intelligence — Board Evidence Digest ({now.strftime('%b %Y')})",
    ...
    for room in rooms:
        try:
            ...
            html = (f"<div style='font:400 14px Arial;color:#1f2937;max-width:560px;margin:auto'>"
                    ...
                    f"<p style='font-size:11px;color:#9ca3af'>You subscribed to weekly updates for this read-only auditor room. Obserra — Control Intelligence.</p></div>")
            for em in subs:
                try:

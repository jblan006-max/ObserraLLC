...
async def weekly_drift_digest(request: Request, background_tasks: BackgroundTasks):
    ...
    from cra_governance import _run_cra_risk_owner_digest
    background_tasks.add_task(_run_cra_risk_owner_digest)
    return {"status": "accepted"}
...
async def daily_drift_digest(request: Request, background_tasks: BackgroundTasks):
    ...
    from agents import _run_agent_room_expiry_reminders, _run_auditor_question_sla_nudge, _run_auditor_question_escalation, _run_board_evidence_digest, _run_scheduled_fire_drills, _run_control_assurance_sla_check
...
async def hourly_overdue_digest(request: Request, background_tasks: BackgroundTasks):
    ...
    from crisis_commander import run_regulatory_clock_alerts, run_scheduled_briefs, run_servicenow_auto_ingest, run_decision_sla_alerts, run_scheduled_sitreps, run_connector_quiet_alerts, run_weekly_director_digest
    ...
    from cra_governance import _run_cra_exec_overview_tick
    background_tasks.add_task(_run_cra_exec_overview_tick)
    from cra_governance import _run_cra_reassess_reminder_tick
    background_tasks.add_task(_run_cra_reassess_reminder_tick)
    from cra_governance import _run_cra_risk_governance_tick
    background_tasks.add_task(_run_cra_risk_governance_tick)
    return {"status": "accepted"}

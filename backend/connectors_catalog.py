...
CATALOG = [
    ...
    # ---- Universal API (point-at-any-endpoint generic connectors) ----
    {"id": "universal-rest", "name": "Universal REST API", "category": "Universal API", "auth": "bearer",
     "capabilities": ["Any HTTPS REST endpoint", "Bearer-token auth", "Live health probe", "Evidence pull"],
     "required_credentials": ["API base URL", "Bearer token"], "token_field": "token", "base_field": "base",
     "fields": [{"key": "base", "label": "API base / health URL", "placeholder": "https://api.your-system.com/health"},
                {"key": "token", "label": "Bearer token", "secret": True}],
     "probe": {"method": "GET", "url": "{base}", "ok_note": "Universal REST API live — endpoint reachable and authorized."},
     "boundary": "Point at any HTTPS REST endpoint; connects when it returns 2xx to the bearer-authenticated probe."},
    {"id": "universal-apikey", "name": "Universal API Key", "category": "Universal API", "auth": "header",
     "capabilities": ["Any HTTPS REST endpoint", "API-key header auth", "Live health probe"],
     "required_credentials": ["API base URL", "API key"], "token_field": "token", "base_field": "base",
     "fields": [{"key": "base", "label": "API base / health URL", "placeholder": "https://api.your-system.com/v1/ping"},
                {"key": "token", "label": "API key", "secret": True}],
     "probe": {"method": "GET", "url": "{base}", "header": "x-api-key", "ok_note": "Universal API-key endpoint live — reachable and authorized."},
     "boundary": "Sends the key in the x-api-key header to any HTTPS endpoint you configure."},
    {"id": "universal-webhook", "name": "Universal Webhook", "category": "Universal API", "auth": "webhook_post",
     "capabilities": ["Outbound event push", "Live delivery test"], "required_credentials": ["Inbound webhook URL"],
     "token_field": "webhook_url",
     "fields": [{"key": "webhook_url", "label": "Webhook URL", "secret": True, "placeholder": "https://your-system.com/hooks/…"}],
     "probe": {"method": "POST", "url": "", "ok_note": "Webhook accepted the Obserra connectivity test post."},
     "boundary": "Posts a test payload to any inbound webhook; connects on a 2xx."},

    # ---- Quality Management (QMS / eQMS for regulated & medical-device products) ----
    {"id": "greenlight-guru", "name": "Greenlight Guru (Medical Device eQMS)", "category": "Quality Management (QMS)", "auth": "bearer",
     "capabilities": ["Design controls", "CAPA", "Complaints", "Risk (ISO 14971)", "Document control"],
     "required_credentials": ["Greenlight Guru API base URL", "API token"], "token_field": "token", "base_field": "base",
     "fields": [{"key": "base", "label": "API base URL", "placeholder": "https://api.greenlight.guru"},
                {"key": "token", "label": "API token", "secret": True}],
     "probe": {"method": "GET", "url": "{base}", "ok_note": "Greenlight Guru live — eQMS API reachable."},
     "boundary": "Medical-device eQMS; connect with a scoped API token from your Greenlight Guru tenant."},
    {"id": "mastercontrol", "name": "MasterControl", "category": "Quality Management (QMS)", "auth": "bearer",
     "capabilities": ["Document control", "CAPA", "Training", "Change control", "Audit"],
     "required_credentials": ["MasterControl API base URL", "OAuth / API token"], "token_field": "token", "base_field": "base",
     "fields": [{"key": "base", "label": "API base URL", "placeholder": "https://acme.mastercontrol.com/api"},
                {"key": "token", "label": "API token", "secret": True}],
     "probe": {"method": "GET", "url": "{base}", "ok_note": "MasterControl live — QMS API reachable."},
     "boundary": "Requires a MasterControl tenant API base + scoped token."},
    {"id": "qualio", "name": "Qualio", "category": "Quality Management (QMS)", "auth": "bearer",
     "capabilities": ["Documents", "CAPA", "Change control", "Suppliers", "Training"],
     "required_credentials": ["Qualio API base URL", "API token"], "token_field": "token", "base_field": "base",
     "fields": [{"key": "base", "label": "API base URL", "placeholder": "https://api.qualio.com/v2"},
                {"key": "token", "label": "API token", "secret": True}],
     "probe": {"method": "GET", "url": "{base}", "ok_note": "Qualio live — QMS API reachable."},
     "boundary": "Cloud eQMS; connect with a Qualio API token scoped to your tenant."},
    {"id": "etq-reliance", "name": "ETQ Reliance", "category": "Quality Management (QMS)", "auth": "bearer",
     "capabilities": ["Nonconformance", "CAPA", "Audit", "Risk", "Document control"],
     "required_credentials": ["ETQ Reliance API base URL", "API token"], "token_field": "token", "base_field": "base",
     "fields": [{"key": "base", "label": "API base URL", "placeholder": "https://acme.etq.com/reliance/rest"},
                {"key": "token", "label": "API token", "secret": True}],
     "probe": {"method": "GET", "url": "{base}", "ok_note": "ETQ Reliance live — QMS API reachable."},
     "boundary": "Requires an ETQ Reliance tenant API base + token."},
    {"id": "veeva-vault-qms", "name": "Veeva Vault QMS", "category": "Quality Management (QMS)", "auth": "bearer",
     "capabilities": ["Quality events", "CAPA", "Change control", "Deviations", "Vault documents"],
     "required_credentials": ["Vault domain", "Session/API token"], "token_field": "token", "base_field": "base",
     "fields": [{"key": "base", "label": "Vault API base URL", "placeholder": "https://acme.veevavault.com/api/v24.1"},
                {"key": "token", "label": "Session / API token", "secret": True}],
     "probe": {"method": "GET", "url": "{base}", "ok_note": "Veeva Vault QMS live — Vault API reachable."},
     "boundary": "Requires a Veeva Vault domain + authenticated session token."},
    {"id": "sparta-trackwise", "name": "Sparta Systems TrackWise Digital", "category": "Quality Management (QMS)", "auth": "bearer",
     "capabilities": ["Quality processes", "CAPA", "Complaints", "Audit"],
     "required_credentials": ["TrackWise Digital tenant + API credentials"], "connectable": False,
     "boundary": "Enterprise QMS; requires a TrackWise Digital tenant API integration."},

    # ---- Labs & Testing (LIMS + medical-device testing / certification labs) ----
    {"id": "labware-lims", "name": "LabWare LIMS", "category": "Labs & Testing (LIMS)", "auth": "bearer",
     "capabilities": ["Samples", "Results", "Batches", "Instruments", "Stability"],
     "required_credentials": ["LabWare LIMS API base URL", "API token"], "token_field": "token", "base_field": "base",
     "fields": [{"key": "base", "label": "API base URL", "placeholder": "https://lims.acme.com/api"},
                {"key": "token", "label": "API token", "secret": True}],
     "probe": {"method": "GET", "url": "{base}", "ok_note": "LabWare LIMS live — API reachable."},
     "boundary": "Requires a LabWare LIMS web/API endpoint + token."},
    {"id": "labvantage", "name": "LabVantage LIMS", "category": "Labs & Testing (LIMS)", "auth": "bearer",
     "capabilities": ["Samples", "Test results", "Workflows", "QC"],
     "required_credentials": ["LabVantage API base URL", "API token"], "token_field": "token", "base_field": "base",
     "fields": [{"key": "base", "label": "API base URL", "placeholder": "https://labvantage.acme.com/api"},
                {"key": "token", "label": "API token", "secret": True}],
     "probe": {"method": "GET", "url": "{base}", "ok_note": "LabVantage live — LIMS API reachable."},
     "boundary": "Requires a LabVantage tenant API base + token."},
    {"id": "starlims", "name": "STARLIMS", "category": "Labs & Testing (LIMS)", "auth": "bearer",
     "capabilities": ["Samples", "Results", "Stability", "QM"],
     "required_credentials": ["STARLIMS REST base URL", "API token"], "token_field": "token", "base_field": "base",
     "fields": [{"key": "base", "label": "REST base URL", "placeholder": "https://starlims.acme.com/rest"},
                {"key": "token", "label": "API token", "secret": True}],
     "probe": {"method": "GET", "url": "{base}", "ok_note": "STARLIMS live — REST API reachable."},
     "boundary": "Requires a STARLIMS REST endpoint + token."},
    {"id": "benchling", "name": "Benchling", "category": "Labs & Testing (LIMS)", "auth": "bearer",
     "capabilities": ["Registry", "Results", "Notebook entries", "Workflows"],
     "required_credentials": ["Benchling tenant base URL", "API key"], "token_field": "token", "base_field": "base",
     "fields": [{"key": "base", "label": "Tenant API base URL", "placeholder": "https://acme.benchling.com/api/v2"},
                {"key": "token", "label": "API key", "secret": True}],
     "probe": {"method": "GET", "url": "{base}", "ok_note": "Benchling live — API reachable."},
     "boundary": "R&D / lab data platform; connect with a Benchling tenant API key."},
    {"id": "thermo-samplemanager", "name": "Thermo Fisher SampleManager LIMS", "category": "Labs & Testing (LIMS)", "auth": "bearer",
     "capabilities": ["Samples", "Results", "Instruments"],
     "required_credentials": ["SampleManager API base + service account"], "connectable": False,
     "boundary": "Enterprise LIMS; requires a SampleManager API endpoint + service account."},
    {"id": "ul-solutions", "name": "UL Solutions (medical-device test lab)", "category": "Labs & Testing (LIMS)", "auth": "upload",
     "capabilities": ["Test reports", "Certification evidence"],
     "required_credentials": ["Lab report upload / customer portal"], "connectable": False,
     "boundary": "Third-party test lab; report evidence ingested via secure upload or the customer portal."},
    {"id": "intertek", "name": "Intertek (test lab)", "category": "Labs & Testing (LIMS)", "auth": "upload",
     "capabilities": ["Test reports", "Certification evidence"],
     "required_credentials": ["Lab report upload / customer portal"], "connectable": False,
     "boundary": "Third-party test lab; report evidence ingested via secure upload or the customer portal."},
    {"id": "tuv-sud", "name": "TUV SUD (notified body / test lab)", "category": "Labs & Testing (LIMS)", "auth": "upload",
     "capabilities": ["Test reports", "Certificates", "Notified-body evidence"],
     "required_credentials": ["Report/certificate upload or portal"], "connectable": False,
     "boundary": "Notified body / test lab; certificates and reports ingested via secure upload or portal."},
    {"id": "eurofins", "name": "Eurofins (medical-device test lab)", "category": "Labs & Testing (LIMS)", "auth": "upload",
     "capabilities": ["Test reports", "Certification evidence"],
     "required_credentials": ["Lab report upload / customer portal"], "connectable": False,
     "boundary": "Third-party test lab; report evidence ingested via secure upload or the customer portal."},
]

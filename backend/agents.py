...
async def evidence_pack(admin: dict = Depends(require_roles("admin"))):
    ...
    lines = [
        "## Attestation",
        f"This evidence pack was generated by Obserra — Control Intelligence for "
...
async def _card_engage_alert(token, doc, event, who=None):
    ...
    html = (f"<div style='font:400 14px Arial;color:#1f2937;max-width:560px;margin:auto'>"
            ...
            f"<p style='font-size:11px;color:#9ca3af'>Obserra — Control Intelligence · Share Center · Engagement alerts</p></div>")
    ...
    for oid in org_ids:
        try:
            ...
            html = (f"<div style='font:400 14px Arial;color:#1f2937;max-width:600px;margin:auto'>"
                    ...
                    f"<p style='font-size:11px;color:#9ca3af'>Obserra — Control Intelligence · Share Center · Weekly engagement digest</p></div>")
    ...
    for org in orgs:
        try:
            ...
            html = (f"<div style='font:400 14px Arial;color:#1f2937;max-width:600px;margin:auto'>"
                    ...
                    f"<p style='font-size:11px;color:#9ca3af'>Obserra — Control Intelligence · Chain-of-custody watchlist</p></div>")
...
async def evidence_room_comment(token: str, body: RoomCommentBody):
    ...
    html = (f"<div style='font:400 14px Arial;color:#1f2937;max-width:560px;margin:auto'>"
            ...
            f"<p style='font-size:11px;color:#9ca3af'>Obserra — Control Intelligence · Defensibility · Auditor questions</p></div>")
...
async def evidence_room_followup(token: str, cid: str, body: RoomFollowupBody):
    ...
    html = (f"<div style='font:400 14px Arial;color:#1f2937;max-width:560px;margin:auto'>"
            ...
            f"<blockquote style='border-left:3px solid #12b4d6;margin:0;padding:6px 14px;color:#374151'>{_html.escape(text[:1000])}</blockquote>"
            f"<p style='font-size:11px;color:#9ca3af'>Obserra — Control Intelligence · Auditor questions</p></div>")
...
# ---- Control Intelligence Executive Board Brief (Resend email + cron cadence) ----
...
def _brief_html(org_name, r):
    ...
    return (
        ...
        '<h2 style="color:#0f1e3d;margin:0">Control Intelligence — Executive Brief</h2>'
        ...
        'are modelled from live Obserra agent records. Sign in to the Control Intelligence for the full '
...
async def _send_ai_brief(org_id):
    ...
    recips = await db.users.find({"org_id": org_id, "role": {"$in": ["admin", "executive"]}},
                                 {"_id": 0, "email": 1}).to_list(200)
    for u in recips:
        await notifications.send_email(u["email"], "Control Intelligence — Executive Brief", html)
    await notifications.create(org_id, "report", "AI security brief delivered",
                               f"Control Intelligence executive brief emailed to {len(recips)} recipient(s).",
                               ref="agentic-ai-security")
    return len(recips)

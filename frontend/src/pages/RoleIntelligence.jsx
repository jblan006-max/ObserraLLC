          ...
          {detail?.loading ? <div className="py-16"><Spinner /></div> : detail && (
            <>
              ...
              <div><h3 className="font-head font-bold text-sm mb-1">Holders ({detail.holders.length})</h3>
                <div className="max-h-40 overflow-y-auto">{detail.holders.map((h) => <div key={h.account_ref} className="text-xs flex justify-between py-0.5 border-b border-border/40"><span>{h.person_name} <span className="font-mono text-muted-foreground">· {h.sap_user}</span></span><span className="text-muted-foreground">{h.system} · {h.status}</span></div>)}</div>
              </div>
            </>
          )}
        </DialogContent>

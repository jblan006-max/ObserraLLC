...
export function NotificationBell() {
  ...
  const openOne = async (n) => {
    if (n.kind === "control_drift") {
      try {
        const { data: wf } = await api.post(`/notifications/${n.id}/remediate`);
        setRemediation(wf);
        setOpen(false);
      } catch { /* ignore */ }
      load();
  ...
  return (
    <>
        ...
        {open && (
          <div data-testid="notif-panel" className="absolute right-0 mt-2 w-80 max-h-[70vh] overflow-y-auto bg-card fact-border rounded-xl shadow-2xl z-50">
            ...
            {data.items.length === 0 ? (
              ...
              return (
                <button key={n.id} data-testid={`notif-item-${n.id}`} onClick={() => openOne(n)}
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      {n.kind === "control_drift" && <span className="text-[10px] font-mono text-ai">Tap to remediate →</span>}
                    </div>
                  </div>
                </button>
              );
      ...
      {remediation && <RemediationModal workflow={remediation} onChanged={load} onClose={() => { setRemediation(null); load(); }} />}
    </>
  );
}

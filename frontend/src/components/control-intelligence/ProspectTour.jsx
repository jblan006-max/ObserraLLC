...
export function ProspectTour({ open, onClose, openTab }) {
  ...
  // Spotlight: track the target element for the current step so the highlight follows it.
  useEffect(() => {
    if (!open) return undefined;
  ...
  return (
    <>
      {rect && !seeding && (
        <div
          data-testid="prospect-tour-spotlight"
          className="fixed z-[9997] pointer-events-none rounded-xl ring-4 ring-med transition-all duration-300"
          style={{
            top: rect.top - 8,
            left: rect.left - 8,
            width: rect.width + 16,

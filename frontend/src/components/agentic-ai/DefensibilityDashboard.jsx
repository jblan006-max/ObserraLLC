...
import { CheckCircle2, Clock, Copy, Database, DoorOpen, FileText, Loader2, MessageSquare, Paperclip, Plus, RefreshCw, Send, Settings2, ShieldCheck, Terminal, Trash2, XCircle } from "lucide-react";
import { toast } from "sonner";
import { DataClassBadge, Panel } from "@/components/agentic-ai/shared";
import { api } from "@/lib/api";

const SOURCE_LABEL = {
  agents: "AI Agent Governance", analytics: "AI Analytics", systems: "AI System Inventory",
  incidents: "AI Incidents", workflows: "Workflow Engine", connectorHealth: "Connector Health",
};
...
function AuditorRoomCard() {
  ...
  const sendDigest = async () => {
    setDigestBusy(true);
    try { const { data } = await api.post("/agents/runtime/board-evidence-digest/send"); toast.success(data.sent ? `Board evidence digest emailed to ${data.sent} recipient(s) with the signed PDF.` : "No recipients found to email."); }
    catch (e) { toast.error(e.response?.data?.detail || "Could not send the board digest."); }
  ...
  return (
    <Panel
      ...
      subtitle="Generate an expiring, no-login link for external auditors to view the live AI Enforcement Evidence Pack, download a watermarked signed PDF (org logo + QR back to this room), and ask questions. Every open is tracked."
      testid="agentic-auditor-room"
      actions={
        <div className="flex flex-wrap items-center gap-2">
          <button data-testid="board-digest-btn" onClick={sendDigest} disabled={digestBusy} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-md border border-ai/40 text-ai text-xs font-head font-bold hover:bg-ai/10 transition-colors disabled:opacity-50">
            {digestBusy ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <FileText className="w-3.5 h-3.5" />} Email board digest
          </button>
          <button data-testid="auditor-room-create-btn" onClick={create} disabled={busy} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-md bg-primary text-primary-foreground text-xs font-head font-bold disabled:opacity-50">
            {busy ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Plus className="w-3.5 h-3.5" />} Create auditor room
          </button>
        </div>
      }
    >
      ...
      {rooms.length === 0 ? (
        ...
        <div className="space-y-2" data-testid="auditor-room-list">
          {rooms.map((room) => (
            <div key={room.token} data-testid={`auditor-room-${room.token}`} className="flex flex-wrap items-center gap-2 rounded-lg border border-border bg-secondary/20 px-3 py-2.5">
              ...
              <span className="text-[10px] font-mono text-muted-foreground inline-flex items-center gap-1"><Clock className="w-3 h-3" /> {room.opens} open(s)</span>
...
function GovernanceSettingsCard() {
  ...
  return (
    <Panel title="Governance settings" subtitle="Choose when the monthly board digest is sent and to whom, and set the auditor-question response SLA." testid="agentic-governance-settings"
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <label className="block">
          <span className="text-[11px] font-mono uppercase tracking-wider text-muted-foreground">Board digest — day of month</span>
          <input data-testid="gov-digest-day" type="number" min={1} max={28} value={s.board_digest_day} onChange={(e) => setS({ ...s, board_digest_day: e.target.value })} className="mt-1.5 w-full bg-secondary/60 rounded-md px-3 py-2.5 text-sm outline-none focus:ring-1 focus:ring-primary" />
        </label>
        <label className="block">
          <span className="text-[11px] font-mono uppercase tracking-wider text-muted-foreground">Auditor-question SLA (hours)</span>
          <input data-testid="gov-sla-hours" type="number" min={1} max={720} value={s.auditor_question_sla_hours} onChange={(e) => setS({ ...s, auditor_question_sla_hours: e.target.value })} className="mt-1.5 w-full bg-secondary/60 rounded-md px-3 py-2.5 text-sm outline-none focus:ring-1 focus:ring-primary" />
        </label>
        <label className="block md:col-span-2">
          <span className="text-[11px] font-mono uppercase tracking-wider text-muted-foreground">Board digest recipients (comma-separated emails — blank = all admins &amp; execs)</span>
          <input data-testid="gov-digest-recipients" value={recips} onChange={(e) => setRecips(e.target.value)} placeholder="board@company.com, ciso@company.com" className="mt-1.5 w-full bg-secondary/60 rounded-md px-3 py-2.5 text-sm outline-none focus:ring-1 focus:ring-primary" />
        </label>
        <label className="flex items-center gap-2 md:col-span-2 cursor-pointer">
          <input data-testid="gov-digest-enabled" type="checkbox" checked={!!s.board_digest_enabled} onChange={(e) => setS({ ...s, board_digest_enabled: e.target.checked })} className="w-4 h-4 accent-ai" />
          <span className="text-sm">Send the monthly board evidence digest automatically</span>
        </label>
      </div>
    </Panel>
  );
  ...
  return (
    <div className={`rounded-md p-2 text-sm ${admin ? "bg-ai/5 border-l-2 border-ai/40 ml-4" : "bg-secondary/30"}`}>
      <div className="flex items-center gap-2 text-[10px] font-mono uppercase tracking-wider text-muted-foreground">
        ...
        {admin ? "Governance" : m.by || "Auditor"} · {fmtDTT(m.at)}
...
function AuditorQuestionsCard() {
  ...
  const setStatus = async (id, status) => {
    try { await api.post("/agents/runtime/evidence-room-comments/status", { id, status }); load(); }
  ...
  return (
    <Panel title="Auditor questions" subtitle="A two-way audit workspace — external auditors' questions, your threaded replies (optionally with the signed evidence PDF), and status. The full Q&A trail is exported into the board digest." testid="agentic-auditor-questions"
      actions={<div className="flex items-center gap-1.5">{sla != null && <span className="text-[10px] font-mono px-2 py-1 rounded-full bg-secondary/60 text-muted-foreground">SLA {sla}h</span>}{overdue > 0 && <span data-testid="auditor-questions-overdue" className="text-[10px] font-mono px-2 py-1 rounded-full bg-crit/10 text-crit">{overdue} overdue</span>}<span className="text-[10px] font-mono px-2 py-1 rounded-full bg-secondary/60 text-muted-foreground">{open} open</span></div>}>
      {comments.length === 0 ? (
            ...
            return (
              <div key={q.id} data-testid={`auditor-question-${q.id}`} className="rounded-lg border border-border bg-secondary/10 p-3">
                <div className="flex items-center gap-2 flex-wrap mb-2">
                  ...
                  {q.overdue && <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-crit/10 text-crit">overdue</span>}
...
function RuntimePlaybooksCard() {
  ...
  return (
    <Panel title="Runtime enforcement playbooks" subtitle="Obserra dispatches this signed webhook on every Suspend / Kill / Resume. Use a per-provider adapter to map it to your agent runtime's stop API." testid="agentic-runtime-playbooks">
      ...
      <div className="space-y-2" data-testid="playbooks-list">
        {(pb.playbooks || []).map((p) => (
          <div key={p.id} data-testid={`playbook-${p.id}`} className="rounded-lg border border-border">
            <button onClick={() => setOpenId(openId === p.id ? null : p.id)} className="w-full flex items-center gap-2 px-3 py-2.5 text-left">
              <Terminal className="w-4 h-4 text-ai shrink-0" />
              <span className="font-head font-bold text-sm">{p.name}</span>
              <span className="ml-auto text-[10px] font-mono text-muted-foreground">{openId === p.id ? "hide" : "view"}</span>
            </button>
            {openId === p.id && (
              <div className="px-3 pb-3 space-y-2">
                ...
                <div className="grid sm:grid-cols-3 gap-2">
                  {Object.entries(p.map || {}).map(([k, v]) => (
                    <div key={k} className="rounded border border-border p-2">
                      <div className="text-[10px] font-mono uppercase text-ai">{k}</div>
                ...
                <div className="rounded bg-secondary/30 p-2">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-[10px] font-mono uppercase text-muted-foreground">adapter</span>
                    <button data-testid={`playbook-copy-${p.id}`} onClick={() => copyText(p.example)} className="inline-flex items-center gap-1 px-2 py-0.5 rounded border border-border text-[10px] hover:bg-secondary"><Copy className="w-3 h-3" /> Copy</button>
                  </div>
            ...
            {connectors.map((connector) => (
              <div key={`${connector.id}:${connector.name}`} className="rounded-lg border border-border p-3">
                <div className="flex items-center justify-between gap-2">
                  <div><div className="font-head font-bold text-sm">{connector.name}</div><div className="text-[10px] text-muted-foreground mt-1">{connector.category}</div></div>
                  <span className={`text-[10px] font-mono ${connector.health === "healthy" ? "text-low" : connector.health === "degraded" ? "text-high" : "text-muted-foreground"}`}>{connector.health || connector.state || "unknown"}</span>
                </div>

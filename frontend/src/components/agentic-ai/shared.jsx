...
const CLASS_STYLE = {
  ...
  "HEURISTIC BASELINE": "bg-high/10 text-high border-high/25",
...
export function StatusPill({ value }) {
  ...
  const style =
    l.includes("critical") || l.includes("autonomous") || l.includes("shadow")
      ? "bg-crit/10 text-crit border-crit/25"
      : l.includes("high") || l.includes("restricted") || l.includes("approval")
      ? "bg-high/10 text-high border-high/25"
      : l.includes("medium") || l.includes("tool assisted")
      ? "bg-med/10 text-med border-med/25"
      : l.includes("sanction") || l.includes("covered") || l.includes("observe")
      ? "bg-low/10 text-low border-low/25"
      : "bg-secondary text-muted-foreground border-border";

  return (
    <span className={`inline-flex px-2 py-0.5 rounded-full border text-[10px] font-mono font-bold capitalize ${style}`}>
      {v}
    </span>
  );
}
...
export function LoadingState() {
  return (
    <div className="min-h-[55vh] flex items-center justify-center" data-testid="agentic-ai-loading">
      <div className="text-center">
        <Loader2 className="w-7 h-7 animate-spin text-primary mx-auto" />
        <div className="text-sm text-muted-foreground mt-3">Loading AI agent security intelligence</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="rounded-xl border border-crit/30 bg-crit/5 p-4 flex items-start gap-3">
      ...
      <div className="min-w-0 flex-1">
        <div className="font-head font-bold text-sm">Live data incomplete</div>
...
export function ProgressBar({ value = 0, accent = "330 81% 60%" }) {
  const v = Math.max(0, Math.min(100, Number(value || 0)));
  return (
    <div className="h-2 bg-secondary/70 rounded-full overflow-hidden">
      <div className="h-full rounded-full" style={{ width: `${v}%`, background: `hsl(${accent})` }} />
    </div>
  );
}